...
import com.kh.spring20.service.PaymentService;
...
import com.kh.spring20.vo.PurchaseListVO;
...
//		return "redirect:/pay/finish";
	...
	public String pay2Purchase(
			@ModelAttribute PurchaseListVO listVO, HttpSession session) throws URISyntaxException {
		...
		for(PurchaseVO purchaseVO : listVO.getPurchase()) {
			ProductDto productDto = productDao.find(purchaseVO.getNo());
			totalAmount += productDto.getPrice() * purchaseVO.getQuantity();
			itemNames.add(productDto.getName());
		}
	...
	public String more(@RequestParam int paymentNo, Model model) throws URISyntaxException {
		//view 페이지로 결제상세내역(List<PaymentDetailDto>),카카오API 조회내역(KakaoPayOrderResponseVO)을 전달한다
